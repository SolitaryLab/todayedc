...
export async function POST(request: Request) {
  ...
  if (!process.env.BLOB_READ_WRITE_TOKEN) return Response.json({ error: "Storage is not connected" }, { status: 503 });
  ...
  const images = await Promise.all(files.map(async (file) => {
    ...
    const blob = await put(`complaints/${Date.now()}-${crypto.randomUUID()}-${safeName}`, file, { access: "public" });
